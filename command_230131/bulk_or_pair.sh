...
输出文件夹"
	...
	echo "------------------------------------"
	echo "bop_derna | 目的:去除比对到某个基因组上的数据|位置:1.fastq_dir 2.name 3.ref_dir&name 4.out_dir"
	echo "------------------------------------" 
}
#---------------------------------------------
...
function bop_derna(){
	#--function:from fq to fq which didn't align ref
	#--input:1.fastq_dir 2.name 3.reference_dir&name(index) 4.out_dir
	#--output:sam bam mapping_picture fq_nomap
	#--need:(eval pair); (eval work_dir(include plot_function.R));  map_stat.R
	fastq_dir=$1
	name=$2
	ref_index=$3
	out_dir=$4
	echo "the fastq is "$fastq_dir/$name"_R1/R2.fq"
	echo "the reference is "$ref_index
	echo "the out_dir is "$out_dir
	hisat2 -p 10 -x $ref_index -1 $fastq_dir/$name"_R1.fq" -2 $fastq_dir/$name"_R2.fq" -S $out_dir/$name".sam" --un-conc $out_dir > $out_dir/$name".log" 2>&1	
	mv $out_dir/un-conc-mate.1 $out_dir/$name"_R1.fq"
	mv $out_dir/un-conc-mate.2 $out_dir/$name"_R2.fq"
	samtools view -bS $out_dir/$name".sam" > $out_dir/$name".bam"
    samtools sort -@ 8 $out_dir/$name".bam" -o $out_dir/$name"_sorted.bam"
    samtools index $out_dir/$name"_sorted.bam"
    rm -rf $out_dir/$name".bam"
